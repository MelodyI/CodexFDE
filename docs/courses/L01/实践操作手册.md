...
动手前，先读[阅读讲义第 4～6 节](./阅读讲义.md#requirement-contract)，理解需求、设计与检查之间的关系，并完成 [L00 环境准备](../L00-课前准备-安装工具与通过环境自检.md)。完整实践可分次完成；每次从上一次记录的未完成步骤继续。
...
完成 [L00 环境准备](../L00-课前准备-安装工具与通过环境自检.md)，打开参考仓库根目录。下面按系统选择一组命令；之后所有标为 `text` 的命令都适用于两类终端。`python` 必须来自参考仓库的 `.venv`。
