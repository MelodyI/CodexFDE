...
```text
〈CodexFDE 控制仓库〉/                ← 第 1～3 节和 course-submit 从这里运行
├─ .venv/
...
└─ .course/experiment-source.json    ← 两个仓库的来源记录
...
![L08 从固定共同起点到 A B C 交接判断的七步实践路线](./assets/l08-practice-flow.svg)
...
| 1. 固定环境与候选 | 控制仓库 | 共同起点 |
| 2. 固定业务裁判 | 你 + 复验者 | 四表预期 |
| 3. 理解 A/B/C | 本地实验 | 证据边界 |
| 4. 建立个人候选 | 控制工作台 | 候选与指纹 |
| 5. 准备 A/B 流程 | 候选仓库 | CI 规则 |
| 6. 完成远程 A/B/C | 远程 CI | Run 与产物 |
| 7. 判断是否交接 | 复验者 | 接受或返工 |
...
<!-- step-guide-4 -->
> **一句话说明：**通过工作台创建个人候选，并记录代码、检查和输入的指纹，保证三次运行可比较。
...
业务实现属于独立 FlowERP；CI 证据记录属于工作台。当前课程候选将两仓库源码组合，合同路径上限为客户业务包 `flowerp/`、按来源区分的 `eval/` 与 `tests/`，以及 `.github/workflows/`、`workbench/ci_evidence.py`、`CI_GATE_SPEC.md`。这些路径相对于返回的隔离候选，不是 CodexFDE 根目录；按来源记录核对后，委托仍须具体到必要文件。由工作台提交实际执行：
