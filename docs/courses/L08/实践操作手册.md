...
请先解释“订单仍是 confirmed”为何不足以通过，再检查个人测试是否比较了全部相关状态。参考脚本默认测试控制仓库；在个人候选中改写为自己的用例，并核对实际导入模块路径，不能用参考结果替代个人代码。
...
当前仓库旧工作流将报告放在隐藏 `.runtime` 中，且普通 Harness 步骤可能在单测失败后跳过。个人候选需要处理这两项接线差异。不得只改讲义中的路径，实际活动 YAML、命令和下载文件也必须对应。
