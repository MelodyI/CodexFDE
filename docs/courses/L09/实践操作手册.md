...
本实践分为参考实验和自己的交付。参考实验帮助理解状态与映射，自己的交付需要实现映射器并修复隔离候选中的取消行为。先读[讲义](阅读讲义.md)，在[提交模板](SUBMISSION.md)写下判断，不把预期结果抄成运行证据。
...
课程外层允许范围包括 `flowerp/`、`agent/`、`eval/`、`tests/`，不等于本次必须允许修改这些目录的全部文件。用[实施提示词](prompts/02-实现有界修复.md)先形成范围决定，再核对实际任务和执行器中的范围。
