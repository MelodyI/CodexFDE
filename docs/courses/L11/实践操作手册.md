...
两个子任务可以并行，主任务整合必须串行。所有角色都使用同一个候选绝对路径，并分别记录允许文件和实际改动。可编辑源文件：[l11-practice-flow.drawio](./assets/l11-practice-flow.drawio)。
