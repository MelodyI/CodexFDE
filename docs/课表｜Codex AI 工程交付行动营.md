...
|16|在新环境接手，并完成现场新需求|容器化、健康检查、回滚意识、演示叙事和技术债说明。|从干净环境启动系统，展示一次成功任务、一次失败打回、一次结果导出和一次反馈写入。|完成结业检查表与五分钟答辩稿。|需求能生成 Spec 和代码；阻断级 Eval 全部通过；Loop 收敛或如实安全退出；Graph 审核结论可追踪；摘要和反馈能够正确生成；干净环境可启动且仓库无密钥。|
